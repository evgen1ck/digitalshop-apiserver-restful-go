...
func (rh *RouteHandler) AuthSignup(w http.ResponseWriter, r *http.Request) {
	...
	if nicknameExist {
		api.RespondWithBadRequest(w, "Nickname: this nickname is already in use")
		return
	}
	if emailExist {
		api.RespondWithBadRequest(w, "Email: this email is already in use")
		return
	}
...
func (rh *RouteHandler) SignupWithToken(w http.ResponseWriter, r *http.Request) {
	...
	if err := tl.Validate(token, tl.IsNotBlank(), tl.IsLen(64), tl.IsNotContainsSpace()); err != nil {
		api.RespondWithBadRequest(w, "Token: "+err.Error())
		return
	}
	...
	if userData.Email == "" {
		api.RespondWithBadRequest(w, "User not found")
		return
	}
	if err != nil {
	...
	api.RespondWithCreated(w, response)
}
